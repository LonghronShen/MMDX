...
namespace MikuMikuDance.Core.Misc
{
    ...
    /// IK計算時の回転軸制限クラス
    /// </summary>
    public class RotationAxisLimit
    {
        /// <summary>
        /// 回転制限を行う軸(X,Y,Z)
        /// </summary>
    ...
    /// 標準IKLimitter
    /// </summary>
    public class DefaltIKLimitter : IIKLimitter
    {
        /// <summary>
        /// 総合稼働軸制限一覧
        /// </summary>
        ...
        public void Adjust(Model.MMDBone bone)
        {
            ...
            //まずはXYZで分解
            if (!MMDXMath.FactoringQuaternionXYZ(bone.LocalTransform.Rotation, out XRot, out YRot, out ZRot))
            {//ジンバルロック対策
                //YZXで分解
                if (!MMDXMath.FactoringQuaternionYZX(bone.LocalTransform.Rotation, out YRot, out ZRot, out XRot))
                {
                    //ZXYで分解
                    MMDXMath.FactoringQuaternionZXY(bone.LocalTransform.Rotation, out ZRot, out XRot, out YRot);
                    FactoringType = 2;
                }
                else
                    FactoringType = 1;
            ...
            if (FactoringType == 0)
                bone.LocalTransform.Rotation = MMDXMath.CreateQuaternionFromRotationMatrix(
                        MMDXMath.CreateRotationXMatrix(XRot) *
                        MMDXMath.CreateRotationYMatrix(YRot) *
                        MMDXMath.CreateRotationZMatrix(ZRot));
                ...
                bone.LocalTransform.Rotation = MMDXMath.CreateQuaternionFromYawPitchRoll(YRot, XRot, ZRot);
