...
namespace MikuMikuDance.Model.Ver1
{
    ...
    public class MMDModel1 : MMDModel
    {
        ...
        public void Write(BinaryWriter writer, float scale)
        {
            ...
            {
                writer.Write((DWORD)Vertexes.Length);
            ...
            {
                writer.Write((DWORD)FaceVertexes.Length);
                for (DWORD i = 0; i < FaceVertexes.Length; i++)
                {
            ...
            {
                writer.Write((DWORD)Materials.Length);
                for (DWORD i = 0; i < Materials.Length; i++)
                {
                    if (Materials[i] == null)
            ...
            if (ToonExpantion)
            {
                ...
                if (PhysicsExpantion)
                {
                    ...
                    {
                        writer.Write((DWORD)RigidBodies.Length);
                        for (long i = 0; i < RigidBodies.Length; i++)
                        {
                    ...
                    {
                        writer.Write((DWORD)Joints.Length);
                        for (long i = 0; i < Joints.Length; i++)
                        {
                            if (Joints[i] == null)
        ...
        internal static byte[] GetBytes(string input, long size)
        {
            ...
            for (long i = 0; i < strs.Length; i++)
                if (i < result.Length)
                    result[i] = strs[i];
            if (result.Length <= strs.Length)
                return result;
            result[strs.Length] = 0;
            for (long i = strs.Length + 1; i < result.Length; i++)
